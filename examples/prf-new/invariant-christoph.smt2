...
(define-fun inv                                        ; function name
  ...
  (let ((bot (as mk-none (Maybe Bits_n)))
		(botint (as mk-none (Maybe Int)))
		(botstuff (as mk-none (Maybe (Tuple2 Int Bits_*)))))

	(let ((r-left (composition-state-Left-__randomness s-left))
		  (r-right (composition-state-Right-__randomness s-right))

										; assignment of tables
		  (TIKL (state-Left-key_top-T (composition-state-Left-key_top s-left)))
		  (TIKR (state-Right-key_top-T (composition-state-Right-key_top s-right)))
		  (TOKR (state-Right-key_bottom-T (composition-state-Right-key_bottom s-right))))
	  (and ; randomness is the same
...
(assert (= return-left ; left function on left state
		   (oracle-Left-prf_left-EVAL old-left handle message)))
(assert (= return-right ; right function on right state
		   (oracle-Right-wrapper-EVAL old-right handle message)))

(declare-const return-value-left (Bits_n))
(declare-const return-value-right (Bits_n))
(declare-const return-state-left CompositionState-Left)
(declare-const return-state-right CompositionState-Right)

(assert (= return-state-left (return-Left-prf_left-EVAL-state return-left)))
(assert (= return-value-left (return-Left-prf_left-EVAL-value return-left)))

(assert (= return-state-right (return-Right-wrapper-EVAL-state return-right)))
(assert (= return-value-right (return-Right-wrapper-EVAL-value return-right)))


; existential quantification
(assert
  (not
	(=> (and (= true (inv old-left old-right))
			 (forall ((n Int) (nn Int))
			   (= (__sample-rand-Left-Bits_n n nn) (__sample-rand-Right-Bits_n n nn))))
		(or
		  (and
			(= mk-abort-Left-prf_left-EVAL return-left)
			(= mk-abort-Right-wrapper-EVAL return-right))

		  (let ((r-left (composition-state-Left-__randomness return-state-left))
				(r-right (composition-state-Right-__randomness return-state-right)))
			(= (state-Left-__randomness-ctr1 r-left)
			   (state-Right-__randomness-ctr1 r-right)))))))
