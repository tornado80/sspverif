...
;;;;;;;;;; removed invariant definition here



;;;;;;;;;; EVAL oracle, argument about both states XXX changes XXX
...
; state of top key packages is identical on left and right on old state
...
; state of top key packages is identical on left old and left new state
(forall ((h Int))           (= (select (state-Left-key_top-T
                            (composition-state-Left-key_top 
                             s-left-new)) h)
...
; state of top key packages is identical on right old and right new state
