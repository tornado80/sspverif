...
%\paragraph{Explicit reductions.}
%When parts of the protocol or some of the hybrid games need to be modified
%at a late point in the proof development, we need to manually backtrack
%which parts of the code equivalence statements are affected, which other
%hybrid games need to be modified consistently etc.. With SMT-solvers in mind, it %seems appealing to explore \emph{automatically} which parts of a proof are %
%affected by small, local modification.

%While, in principle, one could maintain the code of protocols, games, hybrid
%games, reductions and assumption games in software and check which proofs are
%affected (assuming that code-equivalences can be delegate to SMT-solvers) by
%a local change, this practice seems very far from current hand-written
%real-life protocol analysis. Namely, those usually do not give their reductions
%explicitly, since reductions, in essence, emulate the protocol except for one
%small component whose executions are delegated to an oracle by the reduction.
%Therefore, the description of hybrid games usually only contains this modification
%and the reduction is omitted (TODO: put some explicit references here). Thus,
%writing all of these hybrid games and reductions explicitly in their entirety is
%simply not appealing, even if tool-support would help with continuous integration. %(TODO: add some laudable examples such as BHR and the original BR paper as well as others).
...
%For example, (the ideal version of) a key exchange security game has code that samples a uniformly random key, and a symmetric-key-based secure channel security game also samples a uniformly random key. Separating out keys into their separate package $\M{KEYS}$ allows composition

%Thus, we can specify the composition of
%a key exchange protocol and a secure channel by have a key exchange package which
%makes write calls to a $\M{KEYS}$

%Separating out this $\M{KEYS}$ package into its own package

% Concretely, the SSP methodology separates strictly between \emph{code-equivalence statements} and 
%\emph{reduction statements}. A code-equivalence statement turns a game into a functionally equivalent game that is more suitable for a reduction step. A reduction, in turn, is merely defined by a \emph{cut in a graph}, where the cut divides a big game (defined by a call graph of packages) into the (graph of) packages whose code defines the game of the assumption on the one side of the cut, and the (graph of) packages whose code defines the reduction on the other side of the cut.

%Due to code-reuse and strict separation between reduction and code equivalence steps, pen-and-paper proofs that use the SSP 
...
% for formalization and hence, SSPs are the
%first pen-and-paper reduction proof methodology in cryptography (TODO: 
%is that too much?) that can expected to require \emph{less} work with
%tool support than any other reduction methodology developed so far.

%In fact, in the context of SSPs, Halevi's 20-years-old dream vision~\cite{X} 
%of delegating \emph{mundane} proof steps to a computer seems within reach.
