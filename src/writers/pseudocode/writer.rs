...
use crate::package::{OracleSig, OracleDef, Package};
...
impl<W: Write> Writer<W> {
    ...
    pub fn write_statement(&mut self, stmt: &Statement) -> Result {
        ...
        match stmt {
            ...
            Statement::InvokeOracle { id, opt_idx, name, args, target_inst_name } => {
                self.write_identifier(id)?;
                ...
                if let Some(target_inst_name) = target_inst_name {
                    self.write_string(&format!("; /* with target instance name {} */\n", target_inst_name))?;
                } else {
    ...
    pub fn write_oraclesig(&mut self, sig: &OracleSig) -> Result {
        let OracleSig { name, args, tipe } = sig;
